...
app.config['SECRET_KEY'] = 'thisisasecretkeyyoucanmakeitwhateveryouwant'  # this sets a secret key for session management

#  Create a connection to the SQLite3 database
def init_db():  # a function to initialise the database and create the users table if it doesn't exist
    conn = sqlite3.connect('basic_flask2.db')  # connects to the database named basic_flask.db
...
# next lesson explain the call of init_db() function
# add login route
@app.route('/login')
def login():
